...
if __name__ == "__main__":
    ...
    token = args.token
    cv = DatasetDict()
    cv["validation"] = load_dataset("mozilla-foundation/common_voice_13_0", "en", split="validation[:1%]", 
                                    token=token, trust_remote_code=True, 
                                    cache_dir="~/scratch/brdiep/.cache/huggingface/datasets")
    cv["test"] = load_dataset("mozilla-foundation/common_voice_13_0", "en", split="test[:1%]",
                              token=token, trust_remote_code=True, 
                              cache_dir="~/scratch/brdiep/.cache/huggingface/datasets")
